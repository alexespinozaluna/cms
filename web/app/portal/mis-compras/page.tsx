...
import { obtenerMisCompras, PortalError, type Movimiento } from "@/lib/portal-api";
import TablaMovimientos from "@/app/components/portal/TablaMovimientos";

const ROLES_OK = ["Cliente", "Proveedor", "Trabajador"];
...
export default function MisComprasPage() {
  ...
  useEffect(() => {
    ...
    if (!s) {
      router.replace("/login?next=/portal/mis-compras");
      return;
    }
    if (!s.roles.some((r) => ROLES_OK.includes(r))) {
  ...
  const cargar = useCallback(async () => {
    ...
    setCargando(true);
    try {
      setDatos(await obtenerMisCompras(desde, hasta));
    } catch (e) {
      setError(e instanceof PortalError ? e.message : "No se pudieron cargar las compras.");
    } finally {
      setCargando(false);
    }
  }, [desde, hasta]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-12">
      ...
      <h1 className="display mt-2 text-3xl text-verde-osc md:text-4xl">Mis compras</h1>
      ...
      <div className="mt-6">
        {datos && <TablaMovimientos datos={datos} />}
      </div>
    </div>
  );
}
